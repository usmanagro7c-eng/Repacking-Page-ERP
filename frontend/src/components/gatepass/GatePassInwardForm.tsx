...
import type { GatePassData, GatePassRow } from "./GatePassOutwardForm";
...
function initialInwardData(): GatePassData {
  return {
    type: "inward",
    no: "",
    relatedOutwardNo: "",
...
export function GatePassInwardForm({
  ...
  items,
  prefilledData,
}: GatePassInwardFormProps) {
  const [formData, setFormData] = useState<GatePassData>(() => prefilledData || initialInwardData());
  ...
  // Update when prefilledData changes
  useEffect(() => {
    if (prefilledData) {
  ...
  // Load from localStorage on mount if no prefilledData
  useEffect(() => {
    if (!prefilledData) {
      try {
        const saved = localStorage.getItem(INWARD_STORAGE_KEY);
  ...
  const saveLocal = () => {
    try {
      localStorage.setItem(INWARD_STORAGE_KEY, JSON.stringify(formData));
  ...
  const loadLocal = () => {
    try {
      const raw = localStorage.getItem(INWARD_STORAGE_KEY);
      if (raw) {
        ...
        toolbar={
          <>
            ...
            <div className="flex flex-nowrap items-center gap-1.5 shrink-0">
              ...
              >
                {message || "ان ورڈ گیٹ پاس تیار ہے"}
              </span>
            </div>
          </>
        }
        ...
        <div className="a4-sheet mx-auto w-[210mm] max-w-[210mm] bg-white p-7 text-slate-900 shadow-md border-2 border-slate-900 print:shadow-none print:border-2 print:border-black print:p-0">
          ...
          <div className="relative mt-3 text-center border-b border-slate-300 pb-3 pt-1">
            ...
            <div className="absolute left-0 top-1.5 print:hidden">
              <span className="rounded-full px-3 py-0.5 text-xs font-bold bg-blue-100 text-blue-800 border border-blue-300">
                INWARD PASS
              </span>
            </div>
          </div>
          ...
          <section className="mt-4 space-y-3">
            <div className="grid grid-cols-2 gap-x-8 gap-y-3">
              ...
              <div className="flex items-end gap-2">
                ...
                <input
                  value={formData.relatedOutwardNo || ""}
                  onChange={(e) => setField("relatedOutwardNo", e.target.value)}
              ...
              {/* Sender Party */}
              <div className="flex items-end gap-2">
                <span className="urdu shrink-0 text-[15px] font-bold leading-none">بھیجنے والی پارٹی:</span>
                <input
              ...
              {/* Source Warehouse */}
              <div className="flex items-end gap-2">
                <span className="urdu shrink-0 text-[15px] font-bold leading-none">موصولہ از (روانگی):</span>
                <div className="relative w-full">
                  <input
                    list="inward-warehouse-options"
              ...
              <div className="flex items-end gap-2">
                <span className="urdu shrink-0 text-[15px] font-bold leading-none">موصولی گودام (منزل):</span>
                <div className="relative w-full">
                  <input
                    list="inward-warehouse-options"
              ...
              {/* Warehouse Datalist */}
              <datalist id="inward-warehouse-options">
          ...
          <div className="mt-14 grid grid-cols-3 gap-8 text-center">
            {["وصول کنندہ", "گودام انچارج", "انچارج"].map((s) => (
              <div key={s}>
