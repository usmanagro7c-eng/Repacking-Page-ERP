...
export function useProductionForm() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const res = await fetchHelper("/api/production-form/items");
      if (!res.ok) return;
  ...
  const fetchLogo = useCallback(async () => {
    try {
      const res = await fetchHelper("/api/production-form/logo");
      if (!res.ok) return;
  ...
  const fetchConfig = useCallback(async () => {
    try {
      const res = await fetchHelper("/api/production-form/config");
      if (!res.ok) return;
  ...
  const fetchBatches = useCallback(async (itemCode: string, index: 0 | 1 | 2): Promise<string[]> => {
    ...
    try {
      const res = await fetchHelper(`/api/production-form/batches?item=${encodeURIComponent(itemCode)}`);
      if (!res.ok) {
        setBatchOptions([]);
        return [];
      }
  ...
  const fetchUoms = useCallback(async (itemCode: string, index: 0 | 1 | 2): Promise<string[]> => {
    ...
    try {
      const res = await fetchHelper(`/api/production-form/uoms?item=${encodeURIComponent(itemCode)}`);
      if (!res.ok) {
        setUomOptions(["Kg"]);
        return ["Kg"];
      }
  ...
  useEffect(() => {
    const val = data.rawName[0]?.trim();
    if (!val || val.length < 2) return;
    let cancelled = false;
    const timer = setTimeout(() => {
      fetchBatches(val, 0).then((batches) => {
  ...
  useEffect(() => {
    const val = data.rawName[1]?.trim();
    if (!val || val.length < 2) return;
    let cancelled = false;
    const timer = setTimeout(() => {
      fetchBatches(val, 1).then((batches) => {
        if (cancelled || batches.length === 0) return;
        setData((prev) => {
  ...
  useEffect(() => {
    const val = data.readyName?.trim();
    if (!val || val.length < 2) return;
    let cancelled = false;
    const timer = setTimeout(() => {
      fetchBatches(val, 2).then((batches) => {
        if (cancelled || batches.length === 0) return;
        setData((prev) => {
  ...
  const syncToErpnext = useCallback(async () => {
    setIsSyncing(true);
    setMessage("سسٹم میں ڈیٹا منتقل ہو رہا ہے...");
    try {
      const res = await fetchHelper("/api/production-form/sync-erpnext", {
        method: "POST",
      ...
      if (result.success) {
        ...
        setMessage(`✓ سسٹم میں کامیابی سے درج ہو گیا! (دستاویز نمبر: ${result.documentName || "Done"})`);
      } else {
        setMessage(`⚠️ خطاء: ${result.error || "سسٹم میں منتقل نہیں ہو سکا"}`);
      }
    } catch {
      setMessage("⚠️ سرور سے رابطہ نہیں ہو سکا (سرور آف لائن ہے)");
    } finally {
      setIsSyncing(false);
    }
  }, [data, erpnextUrl]);
